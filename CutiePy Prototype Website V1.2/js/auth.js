...
function syncRegisteredAccount() {
    const accounts = getStoredAccounts();
    const account = Object.values(accounts).find(item => item?.email);
    if (account) registeredAccount = account;
...
function openAuthPage(mode) {
    CutiePyPageProgress.navigate(mode ? `auth-form.html?mode=${mode}` : 'auth.html');
}
...
function handlePortalContinue(event) {
    ...
    CutiePyLoader.setButtonLoading(button);
    window.setTimeout(() => openAuthPage('login'), 250);
}
...
function renderAccountPage() {
    ...
    const nextRequirement = completedLessonsCount + 1;
    ...
    progressBar.classList.toggle('is-active', percentage > 0);
    ...
    document.querySelectorAll('.achievement-step').forEach(step => {
        ...
        step.classList.toggle('is-next', Number(step.dataset.requirement) === nextRequirement);
...
function setupAccountForm() {
    ...
    form.addEventListener('submit', event => {
        ...
        const sessionData = JSON.stringify(authState.user);
        if (localStorage.getItem(sessionStorageKey)) {
            localStorage.setItem(sessionStorageKey, sessionData);
        } else {
            sessionStorage.setItem(sessionStorageKey, sessionData);
        }
        const accounts = getStoredAccounts();
        const email = authState.user.email.trim().toLowerCase();
        if (accounts[email]) {
            accounts[email].name = newName;
            localStorage.setItem(accountsStorageKey, JSON.stringify(accounts));
        }
        if (registeredAccount?.email?.trim().toLowerCase() === email) {
...
function enterWebsite() {
    ...
    homeView.classList.add('block', 'view-enter');
    const hashView = window.location.hash.slice(1);
    if (['group', 'challenge', 'study-guide'].includes(hashView)) navigateTo(hashView);
}
