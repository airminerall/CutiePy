...
function ensureAiTutorWidget() {
    ...
    if (!document.querySelector('link[href*="css/style.css"]')) document.head.appendChild(styles);
...
function renderAiTutorTyping() {
    const messages = document.getElementById('ai-tutor-messages');
    if (!messages) return null;
    const message = document.createElement('div');
    message.className = 'ai-tutor-message bot';
    message.appendChild(CutiePyLoader.createDots('loading-dots--chat'));
    messages.appendChild(message);
    messages.scrollTop = messages.scrollHeight;
    return message;
}
...
async function askAiTutor(question) {
    ...
    const typingMessage = renderAiTutorTyping();
...
function toggleAiTutor(forceState) {
    ...
    clearTimeout(tutor.aiTutorCloseTimer);
    tutor.classList.toggle('is-open', isOpen);
    tutor.classList.toggle('is-closing', !isOpen);
    toggle.setAttribute('aria-expanded', String(isOpen));
    if (isOpen) document.getElementById('ai-tutor-input')?.focus();
    else {
        tutor.aiTutorCloseTimer = setTimeout(() => {
            tutor.classList.remove('is-closing');
        }, 260);
    }
}
